...
import { Button, CssBaseline, InputLabel, TextField, ThemeProvider, ToggleButton, ToggleButtonGroup } from '@mui/material';
                ...
                {!submitted ?
                    <div style={{ display: "flex", flexDirection: "row", columnGap:"50px" }}>
                        <ToggleButtonGroup size="small" exclusive value={mode}
                            ...
                            onChange={(_, v) => { if (v) { setMode(v as ModeKey); } }}
                        >
                            <ToggleButton value="llms">LLMs</ToggleButton>
                            <ToggleButton value="llm_apps">Apps</ToggleButton>
                        </ToggleButtonGroup>

                        <TextField label="Page Name" value={pageName} onChange={(e) => {
                            setPageName(e.target.value)
                        }} />
                        <Button onClick={() => { setSubmitted(true) }}>Create page</Button>
                    </div>
                    :
