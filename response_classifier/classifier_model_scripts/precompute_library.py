...
import pickle
import sys
from classifier_model import compute_library_averages
...
TRAIN_DATA_PATH = os.path.join('../data/splits/train.pkl')
VECTORIZER_PATH = os.path.join('../data/processed/tfidf_trigram/train/vectorizer.pkl')
LIBRARY_AVERAGES_PATH = os.path.join(
    '../data/processed/tfidf_trigram/train/library_averages.pkl')
...
def precompute_and_save_library_averages():
    ...
    if not os.path.exists(VECTORIZER_PATH):
        print(
            f"Error: Vectorizer not found at {VECTORIZER_PATH}. Make sure to copy it to the resources folder.")
        sys.exit(1)
