        ...
        if [ -z "$TARGET_FOLDER" ]; then
            for folder in "$DESTINATION"/*/; do
                if [ -d "$folder" ]; then
                    ...
                    (cd "$folder" && eval "promptfoo eval --output ../../outputs/$TIMESTAMP/$folder_no_trailing.json")
                fi
            done
        else
            for folder in "$@"; do
                if [ -d "$DESTINATION/$folder/" ]; then
                    (cd "$DESTINATION/$folder/" && eval "promptfoo eval --output ../../outputs/$TIMESTAMP/$folder.json")
                fi
            done
        fi
